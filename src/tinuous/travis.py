...
class Travis(CISystem):
    ...
    def get_commit(self, build: Dict[str, Any], event_type: EventType) -> Optional[str]:
        if event_type in (EventType.CRON, EventType.PUSH):
            ...
            if pr.merged:
                # Once a PR is merged, the merge_commit_sha changes to the
                # actual merge commit rather than the autogenerated merge
                # commit used by Travis.  We thus check whether the build
                # commit (the autogenerated merge commit) has the PR head as a
                # parent; if so, we assume that the build commit was the
                # autogenerated merge commit for this PR head and return the PR
                # head.
                build_commit = self.ghrepo.get_commit(build["commit"]["sha"])
                if any(c.sha == pr.head.sha for c in build_commit.parents):
                    return pr.head.sha
            # Before a commit is merged, merge_commit_sha is an autogenerated
            # merge commit; we just need to check that it's the same commit as
            # built by Travis*, and, if so, return the PR head commit.
            #
            # * The commits will differ if the PR has been updated since the
            #   build.
            elif pr.merge_commit_sha == build["commit"]["sha"]:
                return pr.head.sha
            log.info("Could not determine PR head commit for build; setting to 'UNK'")
            return None
        else:
