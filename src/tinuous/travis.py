...
class Travis(CISystem):
    ...
    def get_github_commit(self, commit_sha: str) -> Optional[Commit]:
        try:
    ...
    def get_commit(self, build: dict[str, Any], event_type: EventType) -> Optional[str]:
        if event_type in (EventType.CRON, EventType.MANUAL, EventType.PUSH):
            ...
            build_commit = self.get_github_commit(build["commit"]["sha"])
            if build_commit is None or len(build_commit.parents) != 2:
                log.info(
